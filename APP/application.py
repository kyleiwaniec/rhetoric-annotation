...
def save_annotation():
    ...
    # remove the following properties before saaving to DB:
    properties = ["isCurrNode","depth","x","y","id","x0","y0",
                  "feature_id","gpt_response"]
    def removeProps(d): 
        for p in properties:
            if p in d:
                if p == "isCurrNode":
    ...
    if request_json:
        ...
        feature_id    = request_json['feature_id']
        gpt_response  = request_json['gpt_response']
           
        date = time.strftime('%Y-%m-%d %H:%M:%S')
        

        gpt_query = '''INSERT INTO ChatGPT (sentence_id, annotator_id, feature_id, prompt, response, date_updated) VALUES (%s,%s,%s,%s,%s,%s)'''
        cursor.execute(gpt_query, (sentence_id, annotator_id, feature_id, prompt, gpt_response, date))

        removeProps(request_json)

        ann_query = '''INSERT INTO annotations (id, json_string, annotator_id, sentence_id, date_updated) VALUES (%s,%s,%s,%s,%s)
          ON DUPLICATE KEY UPDATE json_string=%s, annotator_id=%s, sentence_id=%s, date_updated=%s;'''

        cursor.execute(ann_query, (annotation_id, json.dumps(request_json), annotator_id, sentence_id, date, 
