...
function d3Tree(treeData,zoomAmount) {
    ...
    function toggleChildren(d) {
        ...
        // set all nodes to isCurrNode = 0
        var self = false;
        if (d["isCurrNode"] == 1){self = true}
        ...
        removeProperty(treeData)

        // Toggle or set to 1
        d["isCurrNode"] = self ? 0 : 1
        d["isCurrNode"] = !d.children ? 0 : d.isCurrNode
        if (!d.children) {
            console.log('name',d,name)
        }
        console.log('d["isCurrNode"]', d.isCurrNode, d.name)
    ...
    function update(source) {
        ...
        var nodeEnter = node.enter().append("g")
            .attr("class", function(d){
                return d.children ? "node" : "node leaf_node";
            })
		...
		nodeEnter.append("rect")
			...
			.attr("x", function(d){return -(d.name.length*10+10)/2})
			.attr("y", -10)
			.attr("width", 0)
			.attr("height", 0)
        ...
        nodeEnter.append("text") // text offset for nodes
            .attr("y", 0)
            .attr("dy", ".35em")
            .attr('class', 'nodeText')
        ...
        node.select("rect.nodeRect")
            .attr("width", function(d) {
                ...
                return d.children || d._children ? d.name.length*10+10 : 0;
            })
